...
describe('TtsService', () => {
  ...
  const batchRepo = {
    find: jest.fn(async () => batches),
    findOne: jest.fn(async ({ where }: { where: { id: string } }) =>
      batches.find((b) => b.id === where.id) || null,
    ),
    create: jest.fn((x: Partial<TtsBatch>) => ({
      id: x.id || `batch-${batches.length + 1}`,
      status: x.status,
      totalJobs: x.totalJobs || 0,
      completedJobs: x.completedJobs || 0,
      failedJobs: x.failedJobs || 0,
      jobIds: x.jobIds || [],
      createdAt: new Date(),
      updatedAt: new Date(),
    })),
    save: jest.fn(async (b: TtsBatch) => {
      const i = batches.findIndex((x) => x.id === b.id);
      if (i >= 0) batches[i] = b;
      else batches.push(b);
      return b;
    }),
  ...
  const ffmpeg = {
    ...
    insertSilence: jest.fn(async (_d: number, _r: number, o: string) => {
      fs.writeFileSync(o, 'silence');
      return o;
    }),
  ...
  const gateway = {
    ...
    emitFailed: jest.fn(),
    emitChunkReady: jest.fn(),
    emitBatchProgress: jest.fn(),
  };
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(TtsBatch), useValue: batchRepo },
