...
3. **Eval lab** — blind **human** CMOS sessions (only Gate 2 certifier). Automated objective prosody proxy is diagnostic only (`meanProxyExpressiveVsPiper`, never labeled CMOS); Gate 2 is **NOT CERTIFIED** until a human ledger exists under `bench/eval/human-sessions/`.
